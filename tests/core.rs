...
fn new_rpc_with_priority() {
    ...
    assert_eq!(
        rpc::Result::Ok(as_json(r#"{ "from": "client2" }"#)),
        rpc.wait().unwrap()
    );
}
...
fn client_disconnects_should_not_stop_handling_of_rpcs() {
    ...
    client3
        .new_rpc(
            ...
            Box::new(TestCall {
                priority: 103,
                result: rpc::Result::Ok(as_json(r#"{ "from": "client3" }"#)),
            }),
        )
        .unwrap();
...
fn call_streaming_rpc_simple() {
            ...
            Box::new(CallbackRpc {
                ...
                callback: |mut context: client::rpc::server::Context, _| {
                    thread::spawn(move || {
                        ...
                        context
                            .finish(rpc::Result::success(&as_json(r#"{ "foo": "blah" }"#)))
                            .unwrap();
                    });
                },
            }),
    ...
    assert_eq!(
        as_json(r#"{ "msg": "three" }"#),
        rpc.recv().unwrap().unwrap()
    );
    assert_eq!(
        rpc::Result::success(as_json(r#"{ "foo": "blah" }"#)),
        rpc.wait().unwrap()
    );
}
                ...
                callback: move |mut context: client::rpc::server::Context, _| {
                    ...
                    thread::spawn(move || {
                        ...
                        while !context.cancelled() {
                            ...
                            let _ =
                                context.update(&as_json(&format!(r#"{{ "value": "{}" }}"#, count)));
                            thread::sleep_ms(10);
                            count += 1
                        }
                        assert!(context
